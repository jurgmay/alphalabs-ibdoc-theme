four51.app.controller('AddressInputCtrl', [
	...
	'Order',
	'LargeAddressList',
	function ($scope, $rootScope, $location, User, Address, Resources, Order, LargeAddressList) {
		$scope.save = function () {
			this.address = $scope.$parent.shipAddressObject;
			this.address.AddressName = this.address.PatientID;
			...
			Address.save(
				...
				function (address) {
					...
					$scope.$parent.showAddressButtons = true;
					UIkit.modal(document.getElementById('address-modal')).hide();
				},
				function (ex) {
		...
		$scope.isPhoneRequired = function () {
			return (
				($scope.user.Permissions.contains('BillingAddressPhoneRequired') && $scope.address.IsBilling) ||
				($scope.user.Permissions.contains('ShipAddressPhoneRequired') && $scope.address.IsShipping) ||
				($scope.user.Permissions.contains('BillingAddressPhoneRequired') && $scope.user.Permissions.contains('ShipAddressPhoneRequired'))
			);
		};
